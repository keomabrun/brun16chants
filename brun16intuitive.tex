...
The low-power wireless network is composed of 18~sensor motes\footnote{2 motes had power issues and are not present on the map.} uniformly distributed between the peach trees, and 3~relay motes to connect the orchard to the gateway some 300~m away.
...
In 3~months of operation, we gathered over than 4~million temperature values, and more than 350,000~network statistics.
...
This is the event we log\footnote{We expect only one \motecreate event per mote. Due to power issues at the manager side, the network restarted a couple of times and new events were created}.
...
The node with the shortest lifetime is {\tt 60-03-82} and has over 5 years of lifetime.
...
\begin{figure}
    ...
    \caption{
        Network stability: the number of \pathdelete and \pathcreate events generated per day over a 16-day period.
